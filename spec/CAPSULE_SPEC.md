...
> **v0.3.12 changes (2026-07-07).** Corpus-driven revision. `domain.annotation` **graduates from the idea queue to a full named domain** (DOMAIN_CAPSULES.md) — a sealed layer of marks (circle / highlight / arrow / strikeout / star / margin-note) over a **digest-pinned** base capsule, with the **anchor-to-truth** rule: a mark's anchor MUST address the base's data block (a `sealed_sources` record, a layer id, or a coordinate in the base's declared CRS), never the regenerable HTML projection. Graduated on compositor's first real strict-valid, digest-resolving annotation capsule and Prospect's parallel gesture runtime (F40); coordinated with the custody layer (base record + annotation record + one seal-verified edge). Validator adds a warn-level `domain.annotation` shape check (base pin present, `base` mirrors `parents[0]`, marks carry id/kind/anchor, anchor type known). This is a named-domain addition, not a Core rule change. Finding F40 in RESEARCH.md; catalogue-scale conformance (compositor's 8-capsule showcase, all strict-valid on this validator) recorded alongside.
...
**Placement (clarified v0.3.13).** Sections are located by their `id`, not by their position in the document. The layout shown above — metadata blocks in `<head>`, root and runtime in `<body>` — is the reference layout producers SHOULD emit; a capsule whose five blocks all sit in `<body>` is equally valid. This matters for capsules published through hosting channels that own the document head: the blocks tunnel through in the body, and consumers recover them by id (F46 in RESEARCH.md).
