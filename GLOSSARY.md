...
### Annotation layer (`domain.annotation`)

A sealed capsule of **marks over another capsule** — circle, highlight, arrow, strikeout, star, margin note — treated as a first-class *second document* (a user's markup is serialized expertise, often more valuable than the base). Graduated to a named domain in spec v0.3.12 (DOMAIN_CAPSULES.md). A normal sealed capsule whose `parents[]` **digest-pins** the base (so a consumer can resolve and verify the exact artifact the marks were made over) and whose data block carries `base` + `marks[]` + `ai_usage_guidance`. The load-bearing rule is **anchor to truth, not projection**: a mark addresses the base's data block (a `sealed_sources` record, a layer id, or a coordinate in the base's declared CRS), never the regenerable HTML — because under `data+manifest` the projection can be re-rendered without changing identity. Live marking is the working layer (a producer runtime); the sealed capsule is the periodic preserve step. First producer: compositor ([F40 in RESEARCH.md](RESEARCH.md)); the persistence form of the markup-as-language thesis.
