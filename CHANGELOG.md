...
### Added — Host mutation requirement + landscape findings (2026-07-27)

- **`spec/HOSTING.md`** — new required section: hosts **MUST** disable CDN HTML post-processing and **SHOULD** set `no-transform` on artifact responses, with the one-command host self-check (fetch your own artifact with a browser User-Agent and confirm it still hashes to its address).
- **F50 in RESEARCH.md** — *the CDN rewrote the capsule.* Deploying a registry behind a mainstream CDN with default settings caused an analytics beacon `<script src>` to be injected into artifact responses **for browser-like requests only**: 4,585 bytes and a matching digest via `curl`, 4,944 bytes and a Rule 2 validation failure via a browser. Byte-identical delivery broken, the artifact no longer a valid capsule, and invisible to every scripted check. The generalizable point: a host cannot detect its own mutation without independently knowing what the artifact should be — content addressing is what made a silent, default-on, browser-only mutation visible in minutes.
- **F51 in RESEARCH.md** — *three host categories, none of which verify.* Reviewed `htmlbin.dev` (agent publishing; excellent device-code + `npx` + agent-skills onboarding; no checksums documented) and `htmlvault.com` (a previously unidentified category: commercial *ephemeral, instrumented distribution* — expiry and per-visit analytics as the product). Extends F21's convergence thesis to three independent hosts that agree on URL shape and raw endpoints and differ on nothing except whether the artifact can be verified — precisely F24's commitment-layer gap.

### Added — The Registry Study: a registry is a format, not a service (2026-07-27)

- **`design/REGISTRY_STUDY.md` (NEW)** — the htmlvault.app registry question answered structurally: instead of a fourth implementation of artifact records behind a web app, the registry is a **specified static directory tree** — layout marker + signed/expiring `index.json` + per-artifact records + content-addressed `objects/sha256/` (the vault's own fan-out) — servable by any dumb host and mirrorable with a copy command. Prior art verified against primary sources (OCI Image Layout's three-part grammar; PEP 503's static-HTML registry with hashes in the reference; TUF's signed-metadata-with-expiry freeze-attack defense). Composes with HOSTING.md's Compliance v1 sketch: the layout makes immutability and integrity *checkable*, shrinking the behavioral contract to availability. Records publish the §8.1 verification pair; `htmlvault publish` becomes a sync of the vault, not an exporter. Working prototype included: five real capsules across three spec lines, verified over zero-logic HTTP, index signature verified, host migration demonstrated with `cp -r`.
- **F49 in RESEARCH.md** — the prototype's tamper test: one byte flipped in a mirrored object was caught instantly by the content address and *correctly ignored* by the capsule seal (the byte was in the projection, which `data+manifest` deliberately excludes — F34). The seal protects meaning; the address protects the published bytes; each catches exactly what the other ignores. Registry records therefore carry `object_digest` and `content_hash` as separate never-conflated fields, and client verification is a three-step MUST: signed index → digest vs address → seal under the declared line's recipe.
...
- **The promised v0.4 removals land** — `capsule_id`, `artifact_id`, `artifact_version`, `related` are rejected on `0.4.x` and keep passing on `0.3.x` forever. Appendix E.1 marked landed, with the refinement it got wrong: deprecation paths can never be *deleted* from a conforming validator, only gated by line.
- **`spec/conformance/hash_vectors.json` (NEW)** — the language-neutral conformance suite F44 demanded: four vectors with manifest, data, canonical forms, payload and expected hash, plus isolated batteries for numbers (as IEEE bit patterns), strings, key ordering, and inputs that MUST error. Implementations MUST pass it to claim conformance. Runners: `compiler/check_conformance.py` and `tools/check-conformance.mjs`.
- **`compiler/jcs.py` (NEW)** — stdlib RFC 8785 canonicalizer, sharing no code with the legacy path so neither can drift into the other. Verified against the RFC's own appendix rows and, in a differential harness, byte-identical to ECMAScript `Number::toString` across 4,031 IEEE doubles.
- **F44's outstanding asks discharged** — recipe v1's number rules are now stated **operationally** (shortest round-trip, the `-4`/`16` exponent thresholds, signed two-digit exponents, the `.0` rule) rather than by reference to a Python interpreter; and the `float_roundtrip` guidance is corrected — it fixes parsing only, and the conformance criterion is passing the vectors.
- **Validator and repair tool dispatch on the declared line** — `VALIDATOR_VERSION` → `0.4.0`; the report names which recipe verified; `JcsError` is reported as a producer conformance failure explicitly distinguished from tampering; `repair_integrity.py` dispatches too, so a 0.4 capsule is never repaired with a hash no verifier accepts. Fixtures in `spec/examples/v04/`.
- **`tools/` (NEW) — the browser toolchain.** `capsule-hash.mjs` implements both recipes with no dependencies and runs unmodified in Node and browsers; `validator.html` is a self-contained drop-a-file integrity checker (F45's highest-leverage missing artifact) that runs the reference vectors on load and states the result in its own interface. Built by `compiler/build_validator.py`, which inlines the module so the page keeps the offline promise the format makes.
- **F47 in RESEARCH.md** — what adopting the standard actually cost: the differential result, the permanence of the imitation tax for the frozen line, and the duplicate-declaration bug that made the validator page inert while looking perfect, which only its real runtime revealed.
...
- **F46 in RESEARCH.md** — a valid capsule was published *through* Claude's artifact channel and recovered: the host wraps the document in a non-portable frame-runtime but preserves author body bytes byte-true; the `data+manifest` hash recomputes bit-identically from the recovered blocks; the served document correctly *fails* validation as-a-whole (the wrapper breaks Rule 2) while extract-by-id + rewrap yields a 30/30 strict capsule with the original hash. Truth-vs-projection (F34) now doing its third job: channel survival. Recovery MUST NOT mutate blocks; recovery provenance belongs in custody records.
- **`design/ARTIFACT_CHANNEL_STUDY.md` (NEW)** — the F41–F46 arc synthesized into direction: the measured served-artifact anatomy, the extractor ecosystem's de-facto export path (localStorage + regex over conversation JSON), the gap matrix (artifact host vs capsule vs vault custody), and the build order — JS §9.1.1 port → two-mode recovery adapter (tunneled extract + bare-artifact wrap) → provenance-preserving extractor feeding htmlvault's inbox.
- **F41–F45 in RESEARCH.md** *(landed 0d6c274, recorded here)* — the Claude-artifact producer population: substrate-without-commitment-layer at population scale (F41), the scraper ecosystem as empirical export demand (F42), the five-non-capsules-one-capsule conversion measurement (F43), `float_roundtrip` necessary-but-not-sufficient + the conformance-distribution failure (F44), and every-path-routes-through-Python (F45).
- `VALIDATOR_VERSION` → `0.3.13` (no behavior change; tracks the doc revision).

### Added — `domain.annotation` graduates; catalogue-scale conformance (spec v0.3.12, 2026-07-07)

- **`domain.annotation` is now a full named domain** (DOMAIN_CAPSULES.md, moved from the idea queue) — a sealed layer of marks over a **digest-pinned** base capsule. A user's markup as a first-class second document (the persistence form of the markup-as-language thesis). Shape: normal sealed capsule; `parents[]` digest-pins the base (`uuid` + `title` + `content_hash`); data block of `base` + `marks[]` + `ai_usage_guidance`; each mark carries `id` / `kind` (circle/highlight/arrow/strikeout/star/margin-note, open set) / `anchor`. **Anchor-to-truth rule:** a mark's anchor MUST address the base's data block — `record` (a `sealed_sources` feature), `layer` (a layer id), or `coordinate` (points in the base's declared CRS) — never the regenerable DOM. Graduated on compositor's first real strict-valid, digest-resolving annotation capsule + Prospect's parallel gesture runtime (F40). Canonical example: `spec/examples/annotation/annotation_example.html`.
- **Validator `domain.annotation` shape check** (`compiler/validate.py`) — warn-level: base pin present in `parents[]`, `data.base` mirrors `parents[0]`, marks carry id/kind/anchor, anchor type is one of `record`/`layer`/`coordinate`. Silent on the `x-*` extension form; fires (strict-fails) on a missing pin or a DOM anchor while integrity still verifies — isolating domain shape from envelope. `VALIDATOR_VERSION` → `0.3.12`.
- **Catalogue-scale conformance verified** — compositor's round-5 showcase (8 capsules: 7 base domain artifacts + 1 annotation layer) all pass the reference validator `--strict`; the annotation capsule's `parents[0].content_hash` resolves against the base's recomputed hash. Not just one capsule conforms — a catalogue does.
- **F40 in RESEARCH.md** — the annotation layer graduates: the produced artifact independently converged on the F39-predicted shape (digest pinning, anchor-to-truth, marks-as-brief), the strongest signal to graduate; the F39→F40 loop (review-without-speccing → producer builds against the named prerequisites → graduation is mechanical) recorded as the discipline paying off.

### Added — Version story, float test vector, digest-pinned parents, annotation review (spec v0.3.11, 2026-07-07)

- **Two-track version story (§8.1)** — `spec_version` declares the normative line (Core, `0.3.0`), never full-spec doc revisions; a conformance statement is the pair (declared line + verifying validator version); custody and provenance records SHOULD store the triple (declared, verified-by, when), and display surfaces SHOULD render `spec 0.3.0 · validated 0.3.11`. Resolves the first production custody run's version-incoherence flag — the wire was right, the vocabulary was missing (F36). The validator's `spec_version` check now states the pair in every report.
- **Test Vector B — floats (§9.1.1)** — implementations MUST parse JSON numbers correctly rounded and MUST reproduce the reference (Python 3 `repr`) number formatting; ECMAScript/RFC 8785 (JCS) formatting is explicitly not this spec's canonical form. The vector reuses Test Vector A's manifest verbatim plus a float-battery data block — the empirical serde_json 1-ULP Web-Mercator ordinate, the `0.30000000000000004` shortest-repr shibboleth, both exponent forms, an integer-valued float — with the reference-derived hash `sha256:47c0aaf0…`. Driven by a false "tampered" verdict in the Rust custody verifier (F37); the finding also records the self-contained-test-vector doctrine (an under-specified vector manufactures false failures — every input byte printed, re-derived by the reference before publication).
- **Digest pinning for `parents[]` (§11.1)** — optional `content_hash` per entry, SHOULD for new capsules: claimed-vs-verifiable applied to provenance edges; the pin is the parent's manifest `content_hash` (scope-aware format identity), not a whole-file digest. Prerequisite for layered artifacts — annotation capsules pin the exact base their marks were made over. `derived_from[].hash` strengthened to SHOULD whenever the source has a stable digest (§11.2).
- **Domain guidance check (`compiler/validate.py`)** — warns when a `domain.*` capsule's data block lacks `ai_usage_guidance` or any of its three required keys. The DOMAIN_CAPSULES.md MUST was previously unenforced, and mintel's production builders ship without the block (F38) — the first unenforced-registry-MUST caught by a producer review. Also: `parents[].content_hash` format check; `VALIDATOR_VERSION` → `0.3.11`.
- **`domain.annotation` in the DOMAIN_CAPSULES.md idea queue** — the markup-as-language annotation layer reviewed and shaped without speculative schema (F39): a normal sealed capsule over a digest-pinned base; anchor-to-truth doctrine (marks address the data block, never the regenerable DOM projection); graduates when Prospect seals its first real annotation capsule.
- **F36–F39 in RESEARCH.md** — version coherence and the claims-need-verification-context pattern (F36); float canonicalization + test-vector doctrine (F37); mintel push-flow review — `x-mintel` as the first production use of the `^x-` extension namespace, inline-producer shape confirming sealed_sources' scope, third unpinned validator consumption path (F38); the annotation layer review (F39).
